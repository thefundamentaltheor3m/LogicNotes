...
\begin{boxtheorem}[Lindenbaum's Lemma]
...
\begin{proof}
    ...
    \end{align*}
    We now show that $\Gamma^*$ is consistent (cf. \Cref{Ch1:Def:Consistent}). Indeed, suppose there exists some formula $\phi_i$ such that $\Gamma \vdash_{\L} \phi_i$ and $\Gamma \vdash_{\L} \parenth{\neg \phi_i}$. Then, by the construction of the $\Gamma_i$, we must have $\Gamma_{i + 1} \vdash_{\L} \phi_i$. Then, we must have $\Gamma_{i + 2} = \Gamma_{i + 1}$, meaning $\Gamma_{}$
\end{proof}
